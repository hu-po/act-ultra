...
def make_ee_sim_env(task_name):
    ...
    if 'sim_transfer_cube' in task_name:
        ...
        xml_path = os.path.join(XML_DIR, f'viperx_ee_pickup_cube.xml')
        physics = mujoco.Physics.from_xml_path(xml_path)
        task = PickupEETask(random=False)
        env = control.Environment(physics, task, time_limit=20, control_timestep=DT,
                                  n_sub_steps=None, flat_observation=False)
    else:
...
class ViperXEETask(base.Task):
    ...
    def initialize_robots(self, physics):
        ...
        # to obtain these numbers:
        # (1) make an ee_sim env and reset to the same start_pose
        # (2) get env._physics.named.data.xpos['vx300s_right/gripper_link']
        #     get env._physics.named.data.xquat['vx300s_right/gripper_link']
...
class PickupEETask(ViperXEETask):
    def __init__(self, random=None):
        super().__init__(random=random)
        self.max_reward = 2 
